...
class RouteReportController extends Controller
{
    ...
    {
        $query = RouteCompanyAllocation::select(
    ...
    /**
     * Driver Performance Report - COMPLETELY FIXED
     */
    public function driverPerformance(Request $request)
    {
        // OPTION 1: Using Route Model with relationships
        ...
        ->leftJoin('routes_new', function($join) use ($request) {
            ...
            // Add date filters in join if needed
            if ($request->filled('date_from')) {
        ...
        ->having('total_routes', '>', 0) // Only show drivers with routes
        ->get();
